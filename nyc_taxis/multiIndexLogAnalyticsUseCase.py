...
def autohisto_agg(args):  # 3
    month_gte = random.randint(9, 10)
    day_gte = random.randint(1, 15)
    month_lte = random.randint(month_gte, 10)
...
def main():
    ...
    start_time = time.time()
    for x in range(1, num_queries + 1):
        runQuery(args, 'date_histogram_agg', date_histogram_agg(args, random.randint(9,10)))
    ...
    with open(save_path + filename, 'a') as csv_file:
        ...
        if len(hit_took_times) > 0:
            csv_file.write(f'All Hit took times : {hit_took_times}\n')
        ...
        if len(miss_took_times) > 0:
            csv_file.write(f'All Miss took times : {miss_took_times}\n')
