...
def distance_amount_agg(args):  # 1
    max_distance = random.randint(1, 50)  # Change the range as needed
    min_distance = random.randint(1, max_distance)
...
def rangeQuery(args):  # 2
    max_distance = random.randint(1, 80)  # Change the range as needed
    min_distance = random.randint(1, max_distance)
...
def autohisto_agg(args):  # 3
    month_gte = random.randint(1, 4)
    day_gte = random.randint(1, 14)
    month_lte = random.randint(month_gte, 4)
...
def runQuery(args, query_name, query, thread_num):
    ...
    #os_hit_count = next(iter(data['nodes'].values()))['indices']['request_cache']['os_hit_count']
    #disk_hit_count = next(iter(data['nodes'].values()))['indices']['request_cache']['disk_hit_count']
    ...
    #    'request_cache']['os_hit_count']  # Check new number of hits
    #disk_new_hits = \
    #next(iter(get_request_cache_stats(args.endpoint, args.username, args.password)['nodes'].values()))['indices'][
    #    'request_cache']['disk_hit_count']  # Check new number of hits
    #if os_new_hits > os_hit_count or disk_new_hits > disk_hit_count:  # If hit count increased
    ...
    #took_times.append(response_time)
...
def worker_thread(thread_num, args, num_queries, hit_took_times, miss_took_times):
    ...
    for x in range(1, num_queries + 1):
        runQuery(args, 'date_histogram_fixed_interval_with_metrics',
                 date_histogram_fixed_interval_with_metrics(args, random.randint(1, 5)), thread_num)
...
def main():
    ...
    with open(save_path + filename, 'a') as csv_file:
        ...
        csv_file.write(f"Total responses {len(response_times)}\n")
        average_response_time = (np.sum(response_times) / len(response_times))
        total_queries = 6 * (int(args.num_queries) * client)
        csv_file.write(f'Aggregated Data of both Hits & Miss\n')
        csv_file.write(f"Total response time: {np.sum(response_times)} \n")
        csv_file.write(f"Average response time: {average_response_time} \n")
        csv_file.write(f"Median response time: {np.median(response_times)} \n")
        csv_file.write(
            f"p99 latency: {round(np.percentile(response_times, 99), 3)} \n")
        csv_file.write(
            f"p95 latency: {round(np.percentile(response_times, 95), 3)} \n")
        csv_file.write(
            f"p90 latency: {round(np.percentile(response_times, 90), 3)} \n")
        csv_file.write(
            f"p50 latency: {round(np.percentile(response_times, 50), 3)} \n")
        csv_file.write(
            f"p75 latency: {round(np.percentile(response_times, 75), 3)} \n")
